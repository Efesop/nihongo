...
export function update(g, callbacks) {
  ...
  if (p.slashTimer <= 0) {
    ...
    // Recovery cooldown after combos
    if (p.slashCombo >= 4 && p.comboWindow > 0) {
      p._slashCooldown = 500; p.slashCombo = 0; p.comboWindow = 0;
    } else if (p.slashCombo >= 2 && p.comboWindow > 0 && !p._hitThisSlash) {
      // This specific slash whiffed — punish wild swinging
      p._slashCooldown = 300; p.slashCombo = 0; p.comboWindow = 0;
    }
    p._hitThisSlash = false; // reset per-slash (not per-combo)
  }
