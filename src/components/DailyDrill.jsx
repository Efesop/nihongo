...
export default function DailyDrill({
  ...
  drillRef,
  submitDrillKana, advanceDrill, startDrill, recordRetrieval,
  setTab,
}) {
  ...
  return <div style={inner}>
    ...
    {drillFlip&&<div style={{display:"flex",gap:10,marginTop:16}}>
      <button onClick={()=>{ recordRetrieval(p[0],false,"phrase-production",null,"drill"); setDrillScore(s=>({...s,w:s.w+1}));advanceDrill();}} style={{...btn,flex:1,padding:14,borderRadius:10,background:c.rs,border:"1px solid "+c.a+"40",color:c.a,fontSize:14,fontWeight:600}}>Missed it</button>
      <button onClick={()=>{ recordRetrieval(p[0],true,"phrase-production",null,"drill"); setDrillScore(s=>({...s,c:s.c+1}));advanceDrill();}} style={{...btn,flex:1,padding:14,borderRadius:10,background:c.gs,border:"1px solid "+c.g+"40",color:c.g,fontSize:14,fontWeight:600}}>Got it</button>
    </div>}
  </div>;
}
